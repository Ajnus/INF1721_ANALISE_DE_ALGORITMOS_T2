...
def get_u(cfg):
    ...
    for u,config in H.items():
        #print(f"u, config: {u}, {config}")
        if hash_cfg(cfg) == config:
            #print(f"{hash_cfg(cfg)} = {config}")
            return u 
    else:
        return None
...
grafo = nx.read_graphml("/home/puc/Documentos/PUC-Rio/INF1721 - Analise de Algoritmos/Trabalho 2/grafo.graphml") 
...
#for u, cfg in H.items():
    #print(f"\n\nNó {u}:\nSeus Nós vizinhos (conectados por uma aresta): {get_neighbors(unhash_cfg(cfg))}")
